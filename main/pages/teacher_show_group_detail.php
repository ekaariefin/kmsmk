  ...
  if($_GET['action'] == 'left_group'){
    if($qother->kick_siswa($_GET['id'],$gid)){
      echo "<script LANGUAGE='JavaScript'>
            window.alert('Berhasil dikeluarkan dari Grup!');
            window.location.href='teacher_show_group_detail.php?gid=".$gid."&ex=".session_id()."';
            </script>";
    }
  }
}
          ...
          <table class="table table-bordered" style="margin-bottom: 30px">
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <a href="teacher_show_group_detail.php?id=<?= $member['user_id']; ?>&action=left_group&gid=<?= $_GET['gid'] ?>&ex=<?= session_id() ?>" type="button" class="btn btn-sm btn-block btn-danger" onclick="return confirm('Apakah anda yakin ingin mengeluarkan akun ini ?')" ><i class="fas fa-sign-out-alt"></i></a>
                </td>
              </tr>
              <?php
                }}
